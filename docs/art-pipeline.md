...
- 曾以本店底图垫图尝试清理吧台伪字，虽成功去字，但重绘导致宽高比与既有导航/装修锚点漂移，自评审未过 8 分；未采用稿与一次性评审文件均未入库。
...
- **路线**：按项目约定跳过本地 `gpt-image-review`，使用内置 ImageGen 直接出图，并由具备读图能力的执行 agent 自评审；仅将通过稿压缩后入库，不保留一次性原图与独立评审文档。
- **风格锚点**：沿用第 1.2 节水彩锚点，并以 `style-reference.png` 与本店成品底图作视觉参考。
- **分店底图 prompt 要点**：空置的 16:9 海边咖啡馆；左侧和后侧临海大窗、漂白木与流木、右侧完整咖啡吧台和糕点柜、四张可用餐桌、中央走道开阔；沙米色、海玻璃蓝与低饱和珊瑚色；禁止人物、动物、文字、标识和 UI。
- **贝壳盏 prompt 要点**：透明背景单物件，海玻璃浅盏、贝壳、短流木与海草，暖纸纹水彩与棕褐铅笔轮廓，无文字、无阴影矩形、无额外物件。
- **藤编灯 prompt 要点**：透明背景单物件，手工藤编灯罩、海蓝玻璃烛灯与低饱和珊瑚色系带，暖纸纹水彩与棕褐铅笔轮廓，无文字、无额外物件。

| 资产 | 轮数 | 自评审 | 后处理与入库 | 结论 |
|---|---:|---|---|---|
| 海风分店底图 | 1 | 构图 9 / 色彩 9.4 / 主题 9.5 | resize 1376×768、WebP q88，316KB | 通过 |
| 海玻璃贝壳盏 | 1 | 构图 9 / 色彩 9 / 主题 9 | 透明通道裁切、宽 96px，6KB | 通过 |
| 藤编海蓝灯 | 1 | 构图 9 / 色彩 9 / 主题 9 | 透明通道裁切、高 112px，9KB | 通过 |

底图、独立导航图、交互热区及两个分店专属装修槽位均已接入；两件普通款式均可用金币购买，不形成碎片独占内容。

## 7. M6 视觉复验后的 2.5D 重设计

### 7.1 设计决策与生成路线

- **复验结论**：原本店把透视空间与大部分家具烘焙进一张底图，角色又沿用早期小比例部件，无法形成可信的前后遮挡与统一人物尺度，因此不再作为本店运行时场景。
- **新视角**：采用 2:1 斜投影的 2.5D 剖切房间，保留后墙与右墙、左下入口、右上吧台和中央主通道；画风统一为暖色水彩与彩铅线稿的可爱绘本风。
- **角色规范**：店主与顾客统一约 1:4 头身，脚底为定位和深度锚点。移动与呼吸继续程序补间，但不再拆成会产生串色和比例漂移的肢体贴片。
- **生成路线**：按项目明确要求不调用本地 `gpt-image-review`，直接使用内置 ImageGen 出图并逐张自评审；仅采用透明通道、视角和比例同时合格的结果。棋盘格被烘进像素的店主四视图与柜台延展稿均淘汰，未入库。

### 7.2 模块拆分与运行时规格

建筑壳体只包含墙面、窗门、木地板与阳光投影；猫窝、储物架、吧台柜体、设备工作站、双人桌、四人桌、店主和顾客均为独立透明素材。运行时统一缩放到 1376×768 设计画布，并按脚底/物件底边的 Y 坐标动态排序。

| 资产 | 轮数 | 自评审结论 | 运行时文件 |
|---|---:|---|---|
| 2.5D 本店概念与空建筑壳体 | 各 1 | 构图、投影与动线通过；壳体无交互家具 | `scene-main-shell.webp`，1376×768，约 147KB |
| 猫窝与储物架 | 各 1 | 透明通道、色调和墙边落点通过 | `scene-cat-nook.webp` / `scene-storage-shelf.webp` |
| 吧台柜体与设备工作站 | 各 1 | 柜体、设备分层，可独立控制遮挡与升级组合 | `scene-counter-base.webp` / `scene-equipment-station.webp` |
| 双人桌与四人桌 | 各 1 | 椅子数清晰，前椅可遮挡人物；用于固定扩建槽位 | `scene-table-two-seat.webp` / `scene-table-four-seat.webp` |
| 1:4 店主与顾客 | 店主 2、顾客 1 | 首轮店主四视图因假透明淘汰；单角色重出后通过 | `owner-1x4.webp` / `customer-1x4.webp` |

扩建不采用自由摆放：每店使用固定桌位与吧台链，所有成本、容量、效率和渲染落点都集中在 `src/config/index.ts`。已购家具永久拥有，可免费收起与摆回，没有施工计时、每日限制或降级惩罚。

### 7.3 穿模复验后的补充资产（2026-09-13）

- **路线**：继续按项目约定使用内置 ImageGen 直接生成并自评审，没有调用 `gpt-image-review`；拒绝稿不入库，也不建立单资产评审 Markdown。
- **本店街景壳体**：以空建筑壳体为垫图，保持 2:1 斜投影、墙体和可走地板完全不动，只补充剖切墙外的低对比街道、树冠、邻里建筑与暖日光。1 轮通过；源稿 `artwork/scene/scene-main-exterior-final.png`，运行时 resize 1376×768、WebP q88，约 156KB。
- **1:4 顾客坐姿**：以现有站姿顾客为身份参考，保持发型、米白针织外套、鼠尾草绿衬衫和深蓝长裤，生成无椅子的正面偏 3/4 坐姿。前 2 轮因棋盘格被烘进像素而拒绝，第 3 轮真实透明通道通过；源稿 `artwork/scene/modules/customer-seated-1x4-final.png`，运行时透明 WebP 约 15KB。
- **接入结论**：顾客进出店使用站姿、到桌后切换坐姿；人物、猫与家具均以落脚点/底边 Y 排序。猫窝与吧台设备重新校准接触线，桌椅增加独立碰撞脚印，避免点地移动和 WASD 穿模。
- **进食动作补充**：收到用户复验后，以通过的顾客坐姿为身份垫图，各 1 轮生成举杯啜饮与手持可颂进食姿态，真实透明通道、服装和比例一致，均一次通过。源稿为 `customer-drinking-1x4-final.png` / `customer-eating-1x4-final.png`，运行时按配方线切换对应透明 WebP，并与基础坐姿交替形成生活化循环。
- **等候动作补充**：同一坐姿身份垫图再生成双手刷手机姿态，1 轮通过；源稿 `customer-phone-1x4-final.png`。有座顾客在选单、等接单、等饮品和等结账时循环刷手机；满座顾客保持站姿，在配置化的吧台队列点等候，空位释放后依次入座。
